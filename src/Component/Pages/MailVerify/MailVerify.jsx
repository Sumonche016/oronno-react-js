import React, { useEffect, useState } from "react";
import video from "../../../assets/video/video.mp4";
import logo from "../../../assets/Images/logo.png";
import { FcOk } from "react-icons/fc";
import { BiErrorCircle } from "react-icons/bi";
import { Link, useNavigate, useParams } from "react-router-dom";
import { useAdminEmailVerifyMutation } from "../../../Redux/auth/authApi";

const MailVerify = () => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  const [mailVerify, setMailVerify] = useState("loading");

  const { hash } = useParams();
  const navigate = useNavigate();

  const [MailVerifyFN, { data, isLoading, isError, error }] =
    useAdminEmailVerifyMutation();

  useEffect(() => {
    MailVerifyFN(hash);
  }, []);

  useEffect(() => {
    if (data?.success) {
      setMailVerify("success");
    }
    if (error) {
      setMailVerify("error");
    }
  }, [data, error]);

  let content = null;

  if (mailVerify == "error") {
    content = (
      <div className="mt-12">
        <BiErrorCircle className="text-6xl mx-auto mb-2 text-primary-red" />
        <h2 className="text-2xl font-semibold text-primary-text text-center capitalize">
          your email verification felid
        </h2>
        <p className="text-lg text-center my-3 font-medium text-primary-text">
          please try again
        </p>
      </div>
    );
  }
  if (mailVerify == "success") {
    setTimeout(() => {
      navigate("/admin/login");
    }, [3000]);

    content = (
      <div className="mt-12">
        <FcOk className="text-6xl mx-auto mb-2" />
        <h2 className="text-2xl font-semibold text-center capitalize">
          your email verification success
        </h2>
        <p className="text-xl text-center my-3 font-medium text-primary-deep-green">
          Redirecting...
        </p>
      </div>
    );
  }
  if (mailVerify == "loading") {
    content = (
      <div className="text-center">
        <div role="status">
          <svg
            aria-hidden="true"
            className="inline w-8 h-8 mr-2 text-gray-200 animate-spin fill-blue-600"
            viewBox="0 0 100 101"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
              fill="currentColor"
            />
            <path
              d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
              fill="currentFill"
            />
          </svg>
          <span className="sr-only">Loading...</span>
        </div>
      </div>
    );
  }
  return (
    <div className="bg-primary-white">
      <div className="container mx-auto">
        {/* div wrap */}
        <div className="w-auto md:h-[calc(100vh_-_64px)] flex justify-center items-center box-border">
          <div className="flex flex-col md:flex-row w-full md:w-[60%] h-auto md:h-[75vh] bg-primary-gray rounded-lg shadow-lg mt-7 md:mt-0">
            {/* video section */}
            <div className="max-w-full min-h-full flex-1 shrink-0 rounded-lg overflow-hidden md:mr-1 relative">
              <video
                className="min-w-full min-h-full object-cover"
                src={video}
                autoPlay
                muted
                loop
              ></video>

              {/* have a account  */}
              <div className="absolute bottom-4 -translate-x-[50%] left-[50%] w-[90%] backdrop-blur-sm bg-white/30  flex justify-between items-center p-3 rounded">
                <p className="text-primary-white font-medium">
                  have an account?
                </p>
                <Link to="/admin/register">
                  <p className="px-3 py-1 bg-primary-gray rounded-md text-primary-deep-green">
                    Register
                  </p>
                </Link>
              </div>
            </div>

            {/* form section  */}
            <div className="flex-1 shrink-0 max-w-full min-h-full md:ml-1 p-3">
              {/* form header  */}
              <div>
                <img
                  className="w-40 h-auto mx-auto mt-4 mb-3"
                  src={logo}
                  alt="logo"
                />
                <h3 className="md:text-xl font-semibold text-primary-text text-center  ">
                  তরুলতা নার্সারি সর্বদা আপনার পশে
                </h3>
              </div>

              <div className="max-w-[300px] mx-auto mt-5">
                {/* register successful */}
                {content}
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default MailVerify;
